...
export default function Seats(props)  {
    ...
    function CostOfTicket() {
        ...
        var res = [];
        for (var i = 0; i< selectingSeats.length; i++) {
            res.push(`${Number(selectingSeats[i].substring(0,1))+1}-${String.fromCharCode(65 + Number(selectingSeats[i].substring(1,2)))}`)
        }
    ...
    function toPayment() {
      ...
      fetch("http://localhost:3001/api/payment/create_payment_url/", {
        method: 'POST',
            ...
            body: JSON.stringify({amount: Number(localStorage.getItem('cost')), 
         ...
         <div className="w3ls-reg" style={{paddingTop: '0px', textAlign: 'center'}}>
                        ...
                        <table className="table-seat" style={{  width: '500px' }}>
                            ...
                            {[...new Array(Number(localStorage.getItem("hallseat")))].map((row, rowIndex) => {
                                ...
                                <div className = "row1" key={rowIndex} >
                                    <tr style={{ width: '20px' }}>{Number(rowIndex)+1}</tr>
                                    <div
                                    key="Klassis-Section"
                                    className=""
